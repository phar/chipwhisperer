...
class AcqKeyTextPattern_CRITTest(AcqKeyTextPattern_Base):
	...
	def initAttackVars(self):
		length = self.keyLen()
		if length <= 32:
			self.attackvars['key'] = util.hexStrToByteArray("01 23 45 67 89 ab cd ef 12 34 56 78 9a bc de f0 23 45 67 89 ab cd ef 01 34 56 78 9a bc de f0 12")[:length]
		else:
			raise ValueError("Invalid key length: %d bytes" % length)
		self.findParam("key").setValue(" ".join(["%02X"%b for b in self.attackvars['key']]), init=True)
	...
	def nextAttackVars(self):
		if self.group1:
			self.group1 = False
			self.attackvars['textin'] = self._textin1

			if AES is not None:
				cipher = AES.new(str(self.attackvars['key']), AES.MODE_ECB)
			...
			self.group1 = True
			self.attackvars['textin'] = self._interleavedPlaintext
